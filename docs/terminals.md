...
`ansi::mode::Mode` and gives `Screen` an enable/disable pair for each,
because a mode you turn on is a mode you owe the terminal back.

## 4. The switches a TUI actually flips

| Mode | Number | What it does | uncurses |
| --- | --- | --- | --- |
| Alternate screen | 1049 | A fresh, scrollback-free buffer for the UI; leaving restores the shell exactly as it was | `enter_alt_screen` / `exit_alt_screen` |
| Cursor visibility | 25 | Hide the blinking cursor while you paint | `show_cursor` / `hide_cursor` |
| Mouse reporting | 1000 / 1006 | Report clicks and motion as input bytes, in a sane encoding | `enable_mouse` / `disable_mouse` |
| Bracketed paste | 2004 | Wrap pasted text so you can tell it apart from typing | `enable_bracketed_paste` / `disable_bracketed_paste` |
| Focus events | 1004 | Tell you when the window gains or loses focus | `enable_focus_events` / `disable_focus_events` |

Trace one. `screen.enter_alt_screen()` stages the bytes `ESC[?1049h` and
flushes. They reach the terminal, which swaps to the alternate buffer. On
...
quietly hijacked their terminal. (`Screen::finish` emits every such
teardown for you.)
...
1. Draw into an in-memory grid of cells. uncurses diffs it against the
   last frame and writes only the bytes that changed (`Canvas`).
...
uncurses makes it one call. The high-level `Screen::finish` emits the
off-sequence for every mode it turned on and returns the line discipline
to where `make_raw` found it. Driving the pieces yourself? It is two:
`Canvas::reset` for the modes, then `Terminal::restore` for raw mode. Run
teardown even when your loop bails out with an error. A clean exit on the
worst day is the difference between a tool people trust and one they
uninstall.

...
This is the governing rule: `Screen` never emits a terminal mode. `Program`
...
return. That keeps capability state, window size, terminal name, origin tracking,
and render-affecting replies up to date. If you take events from
...
`ProgramOptions` now controls only startup behavior that can be emitted without
probing: `bracketed_paste`, `request_pixel_size_on_resize`, `mouse`, and
`track_origin`.

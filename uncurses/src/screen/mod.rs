...
impl<W: Write> Screen<W> {
    ...
    /// Stamp a rich-content rectangle: write `content` as a single
    /// rect-anchor cell at `(rect.x, rect.y)` styled with `style`,
    /// and fill the remainder of the rectangle with rect-body cells.
    ///
    /// The renderer emits the anchor's bytes verbatim when the cell
    /// becomes visible and skips body cells; rows containing rect
    /// cells never participate in scroll detection or in horizontal
    /// insert / delete optimizations.
    ///
    /// Before stamping, the helper:
    ///
    /// * blanks every cell of any *other* rect that overlaps `rect`
    ///   so the leftover cells of the old rect outside the new
    ///   footprint are not orphaned;
    /// * blanks the new rect (which clears any wide-cell pair that
    ///   straddles its edges via [`SurfaceMut::fill_rect`]).
    ///
    /// `rect` is clipped to the screen bounds. An empty clipped
    /// rectangle is a no-op.
    pub fn set_rect_payload(
        &mut self,
        rect: crate::Rect,
        content: impl Into<compact_str::CompactString>,
        style: crate::style::Style,
    ) {
        let bounds = self.front_buf.bounds();
        let clipped = bounds.intersection(rect);
        if clipped.is_empty() {
            return;
        }

        // Collect the distinct old rects that overlap `rect`.
        let mut old_rects: Vec<crate::Rect> = Vec::new();
        for y in clipped.top()..clipped.bottom() {
            for x in clipped.left()..clipped.right() {
                let pos = crate::Position::new(x, y);
                if let Some(c) = self.front_buf.cell(pos)
                    && let Some(r) = c.rect()
                    && r != rect
                    && !old_rects.contains(&r)
                {
                    old_rects.push(r);
                }
            }
        }
        // Blank each old rect entirely so cells of the old rect
        // outside the new rect are not left behind as orphans.
        for old in old_rects {
            self.front_buf.fill_rect(old, &Cell::BLANK);
        }

        // Clear the destination rectangle (handles wide-cell
        // straddlers via the existing fill_rect logic).
        self.front_buf.fill_rect(clipped, &Cell::BLANK);

        let anchor = Cell::rect_anchor(rect, content).with_style(style);
        self.front_buf
            .set_cell(crate::Position::new(rect.x, rect.y), &anchor);
        let body = Cell::rect_body(rect);
        for y in clipped.top()..clipped.bottom() {
            for x in clipped.left()..clipped.right() {
                if x == rect.x && y == rect.y {
                    continue;
                }
                self.front_buf.set_cell(crate::Position::new(x, y), &body);
            }
        }
    }

...
fn filled(w: u16, h: u16, offset: u8) -> RenderBuffer {
    ...
    for y in 0..h {
        for x in 0..w {
            buf.set_cell((x, y), &Cell::narrow(glyph(x, y, offset).to_string()));
        }
    }
    buf
}
...
fn shifted_up_buffer() -> RenderBuffer {
    ...
    for y in 0..HEIGHT {
        for x in 0..WIDTH {
            ...
            buf.set_cell((x, y), &Cell::narrow(glyph(x, source_y, 0).to_string()));
        }
    }
    buf
}
    ...
    for y in 0..HEIGHT {
        ...
        while x + 1 < WIDTH {
            let ch = char::from_u32(0x4E00 + ((x as u32 + y as u32 + offset as u32) % 512))
                .unwrap_or('中');
            buf.set_cell((x, y), &Cell::wide(ch.to_string()));
            x += 2;
        }
    }
    buf
...
fn single_line_change(b: &mut Bencher) {
    ...
    second.clear_touched();
    for x in 0..WIDTH {
        first.set_cell((x, HEIGHT / 2), &Cell::narrow(glyph(x, 0, 0).to_string()));
        second.set_cell((x, HEIGHT / 2), &Cell::narrow(glyph(x, 0, 1).to_string()));
    }
...
fn scattered_tenth_change(b: &mut Bencher) {
    ...
    for y in 0..HEIGHT {
        let mut x = y % 10;
        while x < WIDTH {
            first.set_cell((x, y), &Cell::narrow("0"));
            second.set_cell((x, y), &Cell::narrow("1"));
            x += 10;
        }
    }
...
fn big_single_cell_change(b: &mut Bencher) {
    ...
    first.clear_touched();
    second.clear_touched();
    first.set_cell((BIG_WIDTH / 2, BIG_HEIGHT / 2), &Cell::narrow("0"));
    second.set_cell((BIG_WIDTH / 2, BIG_HEIGHT / 2), &Cell::narrow("1"));
    ...
    b.iter(|| {
        ...
        for y in 0..HEIGHT {
            for x in 0..WIDTH {
                ...
                let cell = Cell::narrow(glyph(x, y, offset).to_string()).style(style);
                s.set_cell((x, y), black_box(&cell));
            }
        }
    });
}
...
fn draw_frame_set_cell(b: &mut Bencher) {
    ...
    b.iter(|| {
        ...
        for y in 0..HEIGHT {
            for x in 0..WIDTH {
                let cell = Cell::narrow(glyph(x, y, offset).to_string());
                s.set_cell((x, y), black_box(&cell));
            }
        }
    });

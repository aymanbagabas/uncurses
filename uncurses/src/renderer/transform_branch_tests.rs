...
fn emit_range_rep_skipped_for_non_ascii() {
    ...
    let mut new_buf = RenderBuffer::new(width, 1);
    for x in 0..8u16 {
        new_buf.set_ref((x, 0), &Cell::narrow('é'));
    }
...
fn transform_el_0_partial_clear_from_column() {
    ...
    let mut new_buf = RenderBuffer::new(width, 1);
    for x in 0..30u16 {
        cur.set_ref((x, 0), &Cell::narrow('A'));
        new_buf.set_ref((x, 0), &Cell::narrow('A'));
    }
    for x in 30..width {
        cur.set_ref((x, 0), &Cell::narrow('X'));
    }
    cur.clear_touched();
...
fn transform_pen_change_emits_sgr() {
    ...
    new_buf.set_ref((0, 0), &Cell::narrow('R').with_style(red));
...
fn transform_osc8_link_open_and_close() {
    ...
    linked_buf.set_ref(
        (0, 0),
        &Cell::narrow('L').with_link("https://example.test", ""),
    );
    ...
    plain_buf.set_ref((0, 0), &Cell::narrow('P'));
...
fn reset_pen_closes_osc8_and_emits_sgr_reset() {
    ...
    renderer
        .update_pen(
            &mut prime,
            Some(&Cell::narrow('L').with_link("https://example.test", "")),
        )
        .unwrap();
    ...
    assert_eq!(
        renderer.cur.link_id(),
        crate::renderer::packed::arena::EMPTY_LINK,
...
fn clear_bottom_with_styled_blank() {
    ...
    let blank = Cell::default().with_style(bg);
    let mut renderer = renderer(width, height, opts_with(|o| o.insert(Optimizations::BCE)));
    ...
    for y in 1..height {
        for x in 0..width {
            new_buf.set_ref((x, y), &blank.clone());
        }
    }

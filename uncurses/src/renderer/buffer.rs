...
impl RenderBuffer {
    ...
    pub(crate) fn cell_ref(&self, pos: impl Into<Position>) -> Option<Cell> {
        self.grid.get(pos.into()).cloned()
    }
    ...
    pub(crate) fn set_ref(&mut self, pos: impl Into<Position>, cell: &Cell) {
        let pos = pos.into();
        let existing = self.grid.get(pos);
    ...
    pub(crate) fn line(&self, y: u16) -> Option<&[Cell]> {
        self.grid.line(y)
    }
    ...
    pub(crate) fn line_mut(&mut self, y: u16) -> Option<&mut [Cell]> {
    ...
    pub(crate) fn insert_lines(&mut self, y: u16, n: u16, bounds_bottom: u16, fill: &Cell) {
    ...
    /// `[y, bounds_bottom)` are marked touched across the full width so
    /// the renderer redraws the affected region on the next frame.
    pub(crate) fn delete_lines(&mut self, y: u16, n: u16, bounds_bottom: u16, fill: &Cell) {
...
impl SurfaceMut for RenderBuffer {
    fn set_cell(&mut self, pos: Position, cell: &Cell) {
        RenderBuffer::set_ref(self, pos, cell);
    }
...
mod tests {
    ...
    fn test_touched_span_expansion() {
        let mut rb = RenderBuffer::new(10, 5);
        rb.set_ref((2, 0), &Cell::narrow('A'));
        rb.set_ref((7, 0), &Cell::narrow('B'));
